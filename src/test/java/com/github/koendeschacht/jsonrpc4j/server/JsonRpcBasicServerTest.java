...
import com.github.koendeschacht.jsonrpc4j.*;
import com.github.koendeschacht.jsonrpc4j.util.CustomTestException;
import com.github.koendeschacht.jsonrpc4j.util.Util;
...
import static com.github.koendeschacht.jsonrpc4j.ErrorResolver.JsonError.METHOD_PARAMS_INVALID;
import static com.github.koendeschacht.jsonrpc4j.JsonRCPBasicServerProtocol.*;
...
public class JsonRpcBasicServerTest {
	...
	public void callMethodWithTooFewParameters() throws Exception {
		...
		assertEquals(METHOD_PARAMS_INVALID.code, decodeAnswer(byteArrayOutputStream).get(ERROR).get(ERROR_CODE).intValue());
	}
	...
	public void receiveInvalidJson() throws IOException {
		...
		assertEquals(ErrorResolver.JsonError.PARSE_ERROR.code, decodeAnswer(byteArrayOutputStream).get(ERROR).get(ERROR_CODE).intValue());
	}
	...
	/**
	 * The {@link com.github.koendeschacht.jsonrpc4j.JsonRpcBasicServer} is able to have an instance of
	 * {@link com.github.koendeschacht.jsonrpc4j.InvocationListener} configured for it.  Prior to a
	...
	public void callConvertedParameterTransformerShouldTransformTheParameters() throws Exception {
		...
		JsonNode json = decodeAnswer(byteArrayOutputStream);
		assertEquals("converted", json.get(RESULT).textValue());
		assertNull(json.get(ERROR));
	}
	...
	public void multiMessageSimple() throws IOException {
		...
		assertEquals(getFromArrayWithId(json, 1).get(RESULT).asText(), param2);
		assertEquals(getFromArrayWithId(json, 2).get(RESULT).asText(), param1);
	...
	public void multiMessageOneOkOneError() throws IOException {
		...
		assertTrue(json.isArray());
		assertEquals(getFromArrayWithId(json, 1).get(RESULT).asText(), param2);
		assertEquals(getFromArrayWithId(json, 2).get(RESULT), null);
		assertFalse(getFromArrayWithId(json, 2).get(ERROR).isNull());
		assertEquals(getFromArrayWithId(json, 2).get(ERROR).get(DATA).get(ERROR_MESSAGE).asText(), param4);
		EasyMock.verify(mockService);
	}
